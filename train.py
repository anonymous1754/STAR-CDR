...
def evaluate_by_app(model, dataloader, device):
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            app_ids = batch['domain_id'].cpu().numpy()
...
def main():
    ...
    model = CDIRecModel(important_features=important_features, categorical_features=categorical_features,
                        continuous_features_dim=continuous_features_dim, sequence_features=sequence_features)

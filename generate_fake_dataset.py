...
def generate_fake_dataset(
    ...
    for domain in domains:
        ...
        for obj in ["user", "item"]:
            for t in ["continue", "concrete"]:
                count = conf[obj][t]
                for i in range(1, count + 1):
                    domain_fields.append(f"{domain}_{obj}_{t}_{i}")
    ...
    special_fields = ["app_id", "userID", "itemID", "scenario_id", "is_click", "is_play", "is_pay"]
    ...
    for idx in range(num_rows):
        ...
        # app_id
        record["app_id"] = app_id_map[current_domain]

        # 基本字段
        record["userID"] = random.randint(10000, 99999)
        record["itemID"] = random.randint(10000, 99999)
